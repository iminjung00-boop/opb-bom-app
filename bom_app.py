...
st.write("BOX 규격, S/W PANEL, 인디케이터 문구 등 모든 제작 사양을 통합 분석합니다.")
...
if uploaded_file:
    ...
    with col_warn1:
        if opb_3t: st.error("🚨 **비표준 사양: OPB 표판 두께 3t 적용 (제작 주의)**")
    ...
    # 4. 🎛️ OPB 및 S/W PANEL 상세 제작 사양 (모든 확인사항 복구)
    st.subheader("🎛️ OPB 및 S/W PANEL 상세 제작 사양")
    
    # BOX 규격 추출 (이미지 속 164 x 1704 등)
    ...
    if all_tables:
        ...
        target_mask = df.astype(str).apply(lambda x: x.str.contains('BUTTON|버튼|HIP|SJ21|PCB|BOARD', case=False, na=False)).any(axis=1)
