...
def version_check():
    if 'version_notified' not in st.session_state:
        ...
        * **자재 필터 보강**: E280A, E281A 등 주요 PCB 및 장치 코드 누락 방지 
        * **기준층 정보 복구**: TOTAL FLOOR 및 기준층 위치 표시 기능 정상화 [cite: 7]
...
if uploaded_file:
    ...
    with col_warn1:
        if parking_val != "미적용":
            st.error(f"🅿️ **기준층 PARKING SW 적용: {parking_val} (제작 주의)** [cite: 7]")
    ...
    total_floors = floor_info_match.group(1).strip() if floor_info_match else "미확인" [cite: 7]
    base_floor_match = re.search(r"기준층\s*[:\s]*([0-9A-Z]+)", all_text)
    base_floor = base_floor_match.group(1).strip() if base_floor_match else "미확인" [cite: 7]
    ...
    opb_type_text = opb_spec_search.group(1).strip() if opb_spec_search else "정보 없음" [cite: 7]
    ...
    aircon_sw = any(k in all_text for k in ["AIR-CON", "에어컨"])
    ...
    if all_tables:
        ...
        for i, row in df_raw.iterrows():
            if any(k in str(row.values) for k in ['품명', 'NAME', '사양', '자재번호']):
                header_idx = i; break
        ...
        # 필터 키워드에 E280, E281 등 주요 관리 번호 추가 
