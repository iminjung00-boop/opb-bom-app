...
st.info("已解决 ValueError 冲突，增强了不同现场 BOM 的兼容性。")
...
if uploaded_file:
    ...
    if all_tables:
        ...
        # [에어 방지 로직] 열 개수에 맞춰 동적으로 제목 부여
        original_cols = df_raw.iloc[header_idx].values
        cn_headers = ["区块(Block)", "材料编号(No)", "材料明细(Description)", "规格(Spec)", "尺寸(Size)", "图纸编号(DWG No)"]
        
        if len(df.columns) <= len(cn_headers):
            df.columns = cn_headers[:len(df.columns)]
        else:
            df.columns = [f"Col_{i}" for i in range(len(df.columns))]

        # 자재 리스트 번역 적용 [cite: 10, 11]
        for col in df.columns:
            df[col] = df[col].apply(translate_content)

        # 2. 데이터 정밀 추출 로직 [cite: 24, 60, 83]
        # (1) 층수 정보
        ...
        # (3) MATERIAL 재질 정보 (요청 포맷 유지) [cite: 83]
        material_info = "无数据"
