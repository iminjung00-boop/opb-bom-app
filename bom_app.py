...
st.write("메인 PCB 설정 및 장애자용 OPB 규격을 최상단에서 즉시 확인합니다.")
...
if uploaded_file:
    ...
    # ---------------------------------------------------------
    # 4. ✨ [신규] 핵심 자재 사양 퀵뷰 (PCB & DIS OPB)
    # ---------------------------------------------------------
    st.subheader("🚀 핵심 자재 사양 퀵뷰")
    
    # E280A16 PC BOARD 사양 추출 (IND:X, SD:O 등)
    pcb_match = re.search(r"E280A16.*?PC\s*BOARD.*?(IND:?[^,\n]+).*?(SD:?[^,\n]+)", all_text, re.DOTALL | re.IGNORECASE)
    pcb_val = f"{pcb_match.group(1)} / {pcb_match.group(2)}" if pcb_match else "정보 없음"
    
    # E281A DISABLE OPB BOX 규격 추출 (484x254 등)
    dis_box_match = re.search(r"E281A.*?DISABLE\s*OPB.*?BOX:?([\d\s*xX,]+)", all_text, re.DOTALL | re.IGNORECASE)
    dis_box_val = dis_box_match.group(1).strip() if dis_box_match else "정보 없음"

    c_quick1, c_quick2 = st.columns(2)
    with c_quick1:
        st.success(f"🖥️ **메인 PCB 설정 (E280A16)**\n\n{pcb_val}")
    with c_quick2:
        st.success(f"♿ **장애자용 OPB BOX SIZE (E281A)**\n\n{dis_box_val}")
    ...
    if all_tables:
        ...
        target_mask = df.astype(str).apply(lambda x: x.str.contains('BUTTON|버튼|HIP|SJ21|PCB|E281A', case=False, na=False)).any(axis=1)
