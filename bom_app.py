...
def show_updates():
    st.info(f"""
    **🚀 {APP_VERSION} 이미지 맞춤형 재질 출력 업데이트 ({LAST_UPDATE})**
    * **출력 형식 일치**: 요청하신 이미지와 동일하게 '* MATERIAL : [재질명]' 형식으로 고정 출력 
    * **텍스트 병합 로직**: MATERIAL 키워드 주변의 흩어진 자재 정보를 하나로 결합하여 노이즈 제거 
    * **핵심 사양 유지**: 층수(FRONT STOP 이전), PCB 옵션, 인승/용량 등 기존 모든 성공 로직 보존 [cite: 24, 25, 70, 83]
    """)
...
if uploaded_file:
    ...
    # 2. 기본 정보 추출
    project = re.search(r"공사명\s*[:\s]+([^\n]+)", all_text).group(1).strip() if "공사명" in all_text else "미확인" [cite: 3, 81]
    unit = re.search(r"호기번호\s*[:\s]+([A-Z0-9]+)", all_text).group(1).strip() if "호기번호" in all_text else "미확인" [cite: 2, 80]
    ...
    if all_tables:
        ...
        # (1) A2000 층수 정보 (FRONT STOP FLOOR 이전까지) [cite: 24, 25]
        ...
        # (3) [이미지 맞춤형] MATERIAL 재질 정보 추출 
        material_info = "정보 없음"
        # 문서 전체를 줄 단위로 검색하여 MATERIAL 포함된 실제 재질 데이터 획득
        lines = all_text.split('\n')
        for i, line in enumerate(lines):
            if "MATERIAL" in line.upper():
                # 다음 줄이나 현재 줄에 재질 관련 키워드 검색
        ...
        with c_w1:
            parking_check = re.search(r"기준층\s*버튼\s*PARKING\s*SW\s*적용\s*\(([^)]+)\)", all_text) [cite: 33]
            parking_val = parking_check.group(1) if parking_check else "미적용"
            if parking_val != "미적용": st.error(f"🅿️ **기준층 PARKING SW 적용: {parking_val}**")
        with c_w2:
            if "면취" in all_text: st.error("🔧 **DIS OPB 하부 면취가공 필수 (C0.5)**") [cite: 83]
            if "비상통화장치" in all_text: st.error("🚨 **비상통화장치 적용 현장**") [cite: 71, 83]
        ...
        with m_c1: 
            st.markdown(f"**🏢 전체 층수 정보 (TOTAL FLOOR)**") [cite: 24]
            st.caption(total_floors_display) 
        with m_c2: st.metric("📍 기준층 위치", base_floor) [cite: 24]
        with m_c3: st.metric("🚪 열림방향", open_direction) [cite: 83]

        st.info(f"👥 **인승/용량:** {name_plate_info}") [cite: 70]

        st.divider()

        st.subheader("🎛️ OPB 및 PCB 상세 제작 사양")
        box_match = re.search(r"BOX\s*[:\s]*([\d\s*xX,]{5,20})", all_text, re.IGNORECASE) [cite: 66, 83]
        sw_dwg = re.search(r"S/W\s*PANEL.*?DWG\s*NO\.?\s*[:\s]*([0-9A-Z]+)", all_text, re.IGNORECASE | re.DOTALL) [cite: 83]
        
        r1_c1, r1_c2, r1_c3, r1_c4 = st.columns(4)
        with r1_c1: st.info(f"✨ **OPB 타입**\n\n{opb_spec}") [cite: 60, 67]
        # 재질 정보를 요청하신 이미지 형식과 100% 동일하게 출력 
        with r1_c2: st.error(f"🎨 **표판 재질 사양**\n\n{material_info}") 
        with r1_c3: st.info(f"📏 **BOX SIZE**\n\n{box_match.group(1).strip() if box_match else '정보 없음'}") [cite: 66, 83]
        with r1_c4: st.info(f"📄 **도면 번호**\n\n{sw_dwg.group(1) if sw_dwg else '정보 없음'}") [cite: 83]

        r2_c1, r2_c2, r2_c3, r2_c4 = st.columns(4)
        with r2_c1: st.info(f"📟 **인디케이터**\n\n{indicator_text}") [cite: 26]
        with r2_c2: st.warning(f"🔋 **PCB 옵션**\n\n{pcb_option}") [cite: 83]
        with r2_c3: st.success(f"❄️ **에어컨:** {aircon}") [cite: 83]
        with r2_c4: st.success(f"⏭️ **오너스킵:** {skip_sw}")

        st.divider()

        st.subheader("🔘 주요 자재 투입 명세 (핵심)")
        target_mask = df.astype(str).apply(lambda x: x.str.contains('BUTTON|버튼|HIP|SJ21|PCB|BOARD|E280|E281|E282', case=False, na=False)).any(axis=1) [cite: 59, 83]
